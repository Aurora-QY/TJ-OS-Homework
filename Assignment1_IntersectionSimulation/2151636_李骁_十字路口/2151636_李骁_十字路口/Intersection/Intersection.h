...
public:
  ...
  void AddVehicle(const int from, const bool is_emergency = false);
  //增加车辆的函数
  ...
  int GetVehicleNum(const int from) const {
    return vehicle_queue_[from].size();
  }
  int GetEmergencyNumInQueue(const int from) const {
    return emergency_num_in_queue[from];
  }
  //获取正在等待的车辆总数、普通车辆数量或紧急车辆数量

  int GetEmergencyNumOnRoad(const int from) const {
    return emergency_num_on_road[from];
  }
  //获取正在路上指定方向行驶的紧急车辆数量

  bool IsRoadAvailable (const int from);
  //获取路面是否可行驶
...
private:
  ...
  mutex queue_RW_[4]; //车辆队列是否正在读写

  int timer_; //红灯与(绿灯加黄灯)的时间(单位：秒)
  int vehicle_id_cnt_;
  int pass_time_; //每辆车通过路口的时间
  int emergency_num_on_road[4]; //四个方向在路上直行紧急车辆(分别为N->S,E->W,S->N,W->E)
  int emergency_num_in_queue[4]; //四个方向在路上等待的紧急车辆(分别为N->S,E->W,S->N,W->E)
  TrafficLight lights_[2];
  vector<Vehicle> vehicle_queue_[4];

  SignalType_GoVehicle
      go_signal_; //传输GoVehicle车辆方向与是否紧急的信号量.四个变量分别为:车辆id、方向、是否紧急、是前段还是后段(1为前端)
